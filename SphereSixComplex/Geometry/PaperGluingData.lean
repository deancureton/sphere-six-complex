...
@[expose] public noncomputable def paperFourPieceStar : FourPieceStarGluingData :=
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData
...
public theorem paperFourPieceStar_nonemptyCentralCollar :
  ...
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData_nonemptyCentralCollar
...
public theorem paperFourPieceStar_connectedPiece :
  ...
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData_connectedPiece
...
public theorem paperFourPieceStar_pathConnectedPiece :
  ...
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData_pathConnectedPiece
...
public theorem paperFourPieceStar_pathConnectedCentralCollar (i : Fin 3) :
  ...
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData_pathConnectedCentralCollar i
...
public noncomputable def paperFourPieceStarBiholomorphicData :
    Geometry.EstablishedBiholomorphicStarGluing.BiholomorphicFourPieceStarData
      paperFourPieceStar :=
  Geometry.establishedPaperAnalyticData.fourPieceStarBiholomorphicData
...
public theorem paperFourPieceStar_pieceSecondCountable :
  ...
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData_pieceSecondCountable
...
public theorem paperFourPieceStar_gluedT2 : T2Space PaperGluedCarrier :=
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData_gluedT2
...
public theorem paperFourPieceStar_gluedCompact : CompactSpace PaperGluedCarrier :=
  Geometry.establishedPaperAnalyticData.fourPieceStarGluingData_gluedCompact
...
@[expose] public noncomputable def paperVanKampenCentralPoint :
    Geometry.establishedPaperAnalyticData.CentralFamily :=
...
@[expose] public noncomputable def paperVanKampenOuterRepresentative :
    RegularTotalSpace Geometry.establishedPaperAnalyticData.periods := by
  let _ := regularFamilyDeckAction Geometry.establishedPaperAnalyticData.periods
  exact quotientSection
    (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta)
    paperVanKampenCentralPoint
...
public theorem paperVanKampenOuterQuotient_isQuotientCoveringMap :
    ...
    IsQuotientCoveringMap
      (quotientProjection
        (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta)) Delta := by
  let hproper : SourceActionProperlyDiscontinuous
      (U := Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization) :=
    sourceActionProperlyDiscontinuous_of_eq
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
  ...
  let _ := familyIsCancelSMul
    (regularParameterMap Geometry.establishedPaperAnalyticData.periods)
  let _ := familyContinuousConstSMul
    (regularParameterMap Geometry.establishedPaperAnalyticData.periods)
    (fun a ↦ (regularPeriodSection_contMDiff Geometry.establishedPaperAnalyticData.periods
  ...
  let _ := familyProperlyDiscontinuousSMul
    ...
    (compactlyUniformPeriods_of_compactUniformLowerBound
      (regularParameterMap Geometry.establishedPaperAnalyticData.periods)
      (regularParameterMap_compactUniformLowerBound Geometry.establishedPaperAnalyticData.periods))
  let _ : LocallyCompactSpace
      (RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) :=
  ...
  let _ : IsCancelSMul Delta
    ...
    regularFamilyDeckAction_isCancelSMul_of_fuchsian
      Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
      hproper
  let _ : ProperlyDiscontinuousSMul Delta
      (RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) :=
    regularFamilyDeckAction_properlyDiscontinuous_of_source
      Geometry.establishedPaperAnalyticData.periods hproper
...
public abbrev PaperVanKampenFiberTorus :=
  FiberTorus Geometry.establishedPaperAnalyticData.periods paperVanKampenRegularBasepoint
...
@[expose] public noncomputable def paperVanKampenCentralToCarrier :
    C(Geometry.establishedPaperAnalyticData.CentralFamily, PaperGluedCarrier) :=
  ⟨paperFourPieceStar.glueData.toGlueData.ι none,
...
public theorem paperVanKampenCentralToCarrier_basepoint :
    paperVanKampenCentralToCarrier
        (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
          paperVanKampenOuterRepresentative) =
...
@[expose] public noncomputable def paperVanKampenMeridianLiftPath (g : Delta) :
    Path paperVanKampenOuterRepresentative
      (regularFamilyDeckMap Geometry.establishedPaperAnalyticData.periods g
        paperVanKampenOuterRepresentative) :=
  regularDeckPath Geometry.establishedPaperAnalyticData.periods paperVanKampenOuterRepresentative g
...
@[expose] public noncomputable def paperVanKampenDeckMeridianLoop (g : Delta) :
  ...
  ((regularDeckLoop Geometry.establishedPaperAnalyticData.periods
...
public theorem paperVanKampenCentralFamily_outerDeckAndCoverRange_generate_at
    (x : RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) :
    let _ := regularFamilyDeckAction Geometry.establishedPaperAnalyticData.periods
    Subgroup.closure
      (({regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₁,
        regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₂} :
          Set (FundamentalGroup Geometry.establishedPaperAnalyticData.CentralFamily
            (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods x))) ∪
        (FundamentalGroup.mapOfEq
          (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods)
          (x := x) (y := regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods x)
          rfl).range) = ⊤ := by
  let _ := regularFamilyDeckAction Geometry.establishedPaperAnalyticData.periods
  let _ : PathConnectedSpace
      (RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) :=
    regularTotalSpace_pathConnected Geometry.establishedPaperAnalyticData.periods
  let hp := paperVanKampenOuterQuotient_isQuotientCoveringMap
  let S : Set (FundamentalGroup Geometry.establishedPaperAnalyticData.CentralFamily
      (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods x)) :=
    {regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₁,
      regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₂}
  have hmono (g : Delta) :
      hp.fundamentalGroupToMulOpposite
          (⟨x, rfl⟩ : (quotientProjection
              (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta)) ⁻¹'
            {quotientProjection
              (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta) x})
          (regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g) =
        MulOpposite.op g := by
    change hp.fundamentalGroupToMulOpposite _
      (pathLoopClass (projectedOrbitDeckPath x g
        (regularDeckPath Geometry.establishedPaperAnalyticData.periods x g))) = _
    exact fundamentalGroupToMulOpposite_projectedOrbitDeckPath hp x g
      (regularDeckPath Geometry.establishedPaperAnalyticData.periods x g)
  have himage : Subgroup.closure
      (hp.fundamentalGroupToMulOpposite
        (⟨x, rfl⟩ : (quotientProjection
            (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta)) ⁻¹'
          {quotientProjection
            (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta) x}) '' S) = ⊤ := by
    rw [show hp.fundamentalGroupToMulOpposite
          (⟨x, rfl⟩ : (quotientProjection
              (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta)) ⁻¹'
            {quotientProjection
              (M := RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) (G := Delta) x}) '' S =
      ...
      · rintro (rfl | rfl)
        · exact ⟨regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₁,
            Or.inl rfl, hmono g₁⟩
        · exact ⟨regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₂,
            Or.inr rfl, hmono g₂⟩]
...
public theorem paperVanKampenCentralFamily_outerDeckAndCoverRange_generate :
    let _ := regularFamilyDeckAction Geometry.establishedPaperAnalyticData.periods
    Subgroup.closure
      (({regularDeckMeridian Geometry.establishedPaperAnalyticData.periods
        ...
        (FundamentalGroup.mapOfEq
          ...
          (y := regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
...
public abbrev paperVanKampenOrderThreeRadius : ℝ :=
  Geometry.establishedPaperAnalyticData.starSeparation.orderThree.radius

public abbrev paperVanKampenOrderFourRadius : ℝ :=
  Geometry.establishedPaperAnalyticData.starSeparation.orderFour.radius
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarPoint :
    Geometry.establishedPaperAnalyticData.starCollarSourceType 1 :=
  Classical.choice (Geometry.establishedPaperAnalyticData.starCollarSource_nonempty 1)

@[expose] public noncomputable def paperVanKampenOrderFourCollarPoint :
    Geometry.establishedPaperAnalyticData.starCollarSourceType 2 :=
  Classical.choice (Geometry.establishedPaperAnalyticData.starCollarSource_nonempty 2)
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarRepresentative :
    (orderThreeAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
      paperVanKampenOrderThreeRadius).carrier := by
  let _ := restrictedMulAction
    (orderThreeAffineFamilyAction Geometry.establishedPaperAnalyticData.periods)
    (orderThreeAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
      paperVanKampenOrderThreeRadius)
  exact quotientSection
    (M := (orderThreeAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
...
@[expose] public noncomputable def paperVanKampenOrderFourCollarRepresentative :
    (orderFourAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
      paperVanKampenOrderFourRadius).carrier := by
  let _ := restrictedMulAction
    (orderFourAffineFamilyAction Geometry.establishedPaperAnalyticData.periods)
    (orderFourAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
      paperVanKampenOrderFourRadius)
  exact quotientSection
    (M := (orderFourAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarCoverRepresentative :
  ...
  Classical.choose
    (Geometry.establishedPaperAnalyticData.orderThreePuncturedCoverMap_surjective
...
public theorem paperVanKampenOrderThreeCollarCoverRepresentative_projects :
    Geometry.establishedPaperAnalyticData.orderThreePuncturedCoverMap
        paperVanKampenOrderThreeRadius
...
@[expose] public noncomputable def paperVanKampenOrderFourCollarCoverRepresentative :
  ...
  Classical.choose
    (Geometry.establishedPaperAnalyticData.orderFourPuncturedCoverMap_surjective
...
public theorem paperVanKampenOrderFourCollarCoverRepresentative_projects :
    Geometry.establishedPaperAnalyticData.orderFourPuncturedCoverMap
        paperVanKampenOrderFourRadius
...
public theorem paperVanKampenOrderThreeCollarCoverGenerator_iterate_three :
    (Geometry.establishedPaperAnalyticData.orderThreeAffinePuncturedCoverGenerator)^[3]
        paperVanKampenOrderThreeCollarCoverRepresentative =
      Geometry.establishedPaperAnalyticData.orderThreeAffinePuncturedCoverPeriodTranslate
        paperVanKampenOrderThreeCollarCoverRepresentative :=
  Geometry.establishedPaperAnalyticData.orderThreeAffinePuncturedCoverGenerator_iterate_three
    paperVanKampenOrderThreeCollarCoverRepresentative
...
public theorem paperVanKampenOrderFourCollarCoverGenerator_iterate_four :
    (Geometry.establishedPaperAnalyticData.orderFourAffinePuncturedCoverGenerator)^[4]
        paperVanKampenOrderFourCollarCoverRepresentative =
      Geometry.establishedPaperAnalyticData.orderFourAffinePuncturedCoverPeriodTranslate
        paperVanKampenOrderFourCollarCoverRepresentative :=
  Geometry.establishedPaperAnalyticData.orderFourAffinePuncturedCoverGenerator_iterate_four
    paperVanKampenOrderFourCollarCoverRepresentative
...
public theorem paperVanKampenOrderThreeCollarCoverPower_homotopic_periodPath :
    Path.Homotopic
      (Geometry.establishedPaperAnalyticData.orderThreeAffinePuncturedCoverLiftPathThreeToFull
        paperVanKampenOrderThreeCollarCoverRepresentative)
      (Geometry.establishedPaperAnalyticData.orderThreeFullCoverPeriodPath
        paperVanKampenOrderThreeCollarCoverRepresentative) :=
  Geometry.establishedPaperAnalyticData
    |>.orderThreeAffinePuncturedCoverLiftPathThree_homotopic_periodPath
      Geometry.establishedPaperAnalyticData.starSeparation.orderThree.radius_pos
      Geometry.establishedPaperAnalyticData.starSeparation.orderThree.radius_lt_one
      paperVanKampenOrderThreeCollarCoverRepresentative
...
public theorem paperVanKampenOrderFourCollarCoverPower_homotopic_periodPath :
    Path.Homotopic
      (Geometry.establishedPaperAnalyticData.orderFourAffinePuncturedCoverLiftPathFourToFull
        paperVanKampenOrderFourCollarCoverRepresentative)
      (Geometry.establishedPaperAnalyticData.orderFourFullCoverPeriodPath
        paperVanKampenOrderFourCollarCoverRepresentative) :=
  Geometry.establishedPaperAnalyticData
    |>.orderFourAffinePuncturedCoverLiftPathFour_homotopic_periodPath
      Geometry.establishedPaperAnalyticData.starSeparation.orderFour.radius_pos
      Geometry.establishedPaperAnalyticData.starSeparation.orderFour.radius_lt_one
      paperVanKampenOrderFourCollarCoverRepresentative
...
public theorem paperVanKampenOrderThreeFillingSourcePower_homotopic_periodPath :
    Path.Homotopic
      ((Geometry.establishedPaperAnalyticData
        ...
        (Geometry.establishedPaperAnalyticData.orderThreeFillingCoverMap_continuous
          paperVanKampenOrderThreeRadius))
      ((Geometry.establishedPaperAnalyticData.orderThreeFullCoverPeriodPath
          paperVanKampenOrderThreeCollarCoverRepresentative).map
        (Geometry.establishedPaperAnalyticData.orderThreeFillingCoverMap_continuous
...
public theorem paperVanKampenOrderFourFillingSourcePower_homotopic_periodPath :
    Path.Homotopic
      ((Geometry.establishedPaperAnalyticData
        ...
        (Geometry.establishedPaperAnalyticData.orderFourFillingCoverMap_continuous
          paperVanKampenOrderFourRadius))
      ((Geometry.establishedPaperAnalyticData.orderFourFullCoverPeriodPath
          paperVanKampenOrderFourCollarCoverRepresentative).map
        (Geometry.establishedPaperAnalyticData.orderFourFillingCoverMap_continuous
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarDeckPath :
  ...
  let _ := restrictedMulAction
    ...
    (orderThreeAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
      paperVanKampenOrderThreeRadius)
  exact (Geometry.establishedPaperAnalyticData.orderThreeAffinePuncturedCoverDeckPath
...
@[expose] public noncomputable def paperVanKampenOrderFourCollarDeckPath :
  ...
  let _ := restrictedMulAction
    ...
    (orderFourAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
      paperVanKampenOrderFourRadius)
  exact (Geometry.establishedPaperAnalyticData.orderFourAffinePuncturedCoverDeckPath
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarToRegular :
    C((orderThreeAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
        Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
        paperVanKampenOrderThreeRadius).carrier,
      RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) := by
  let _ := Geometry.establishedPaperAnalyticData.totalSpaceCharts
  let _ : IsManifold GlobalDeckTotalModel ω
      (TotalSpace (parameterMap Geometry.establishedPaperAnalyticData.periods)) :=
    Geometry.establishedPaperAnalyticData.totalSpace_isManifold_analytic
  let hsource := Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
  let hproper : SourceActionProperlyDiscontinuous
      (U := Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization) :=
    sourceActionProperlyDiscontinuous_of_eq hsource
  let gauge := orderThreePuncturedCollarGaugeDiffeomorph
    Geometry.establishedPaperAnalyticData.periods
    Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
    paperVanKampenOrderThreeRadius
  exact ⟨fun q => orderThreeCollarToRegular Geometry.establishedPaperAnalyticData.periods hproper
      Geometry.establishedPaperAnalyticData.starSeparation.orderThree.sourceData (gauge q),
    (orderThreeCollarToRegular_isOpenEmbedding Geometry.establishedPaperAnalyticData.periods hproper
      Geometry.establishedPaperAnalyticData.starSeparation.orderThree.sourceData).continuous.comp
        gauge.continuous⟩

@[expose] public noncomputable def paperVanKampenOrderFourCollarToRegular :
    C((orderFourAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods
        Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization_sourceAction
        paperVanKampenOrderFourRadius).carrier,
      RegularTotalSpace Geometry.establishedPaperAnalyticData.periods) := by
  let _ := Geometry.establishedPaperAnalyticData.totalSpaceCharts
  ...
  let gauge := orderFourPuncturedCollarGaugeDiffeomorph
    ...
    Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
    paperVanKampenOrderFourRadius
  exact ⟨fun q => orderFourCollarToRegular Geometry.establishedPaperAnalyticData.periods hproper
      Geometry.establishedPaperAnalyticData.starSeparation.orderFour.sourceData (gauge q),
    (orderFourCollarToRegular_isOpenEmbedding Geometry.establishedPaperAnalyticData.periods hproper
      Geometry.establishedPaperAnalyticData.starSeparation.orderFour.sourceData).continuous.comp
        gauge.continuous⟩
...
public theorem paperVanKampenOrderThreeCollarToRegular_generator :
  ...
  let _ := restrictedMulAction
    (orderThreeAffineFamilyAction Geometry.establishedPaperAnalyticData.periods)
    (orderThreeAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods hsource
      paperVanKampenOrderThreeRadius)
  change orderThreeCollarToRegular Geometry.establishedPaperAnalyticData.periods hproper
      Geometry.establishedPaperAnalyticData.starSeparation.orderThree.sourceData
        (orderThreePuncturedCollarGaugeDiffeomorph Geometry.establishedPaperAnalyticData.periods
          Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
          paperVanKampenOrderThreeRadius
          (restrictedActionMap
            (orderThreeAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods hsource
            ...
            (cyclicGenerator 3) paperVanKampenOrderThreeCollarRepresentative)) =
    regularFamilyDeckMap Geometry.establishedPaperAnalyticData.periods g₁
      (orderThreeCollarToRegular Geometry.establishedPaperAnalyticData.periods hproper
        Geometry.establishedPaperAnalyticData.starSeparation.orderThree.sourceData
          (orderThreePuncturedCollarGaugeDiffeomorph Geometry.establishedPaperAnalyticData.periods
            Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
            paperVanKampenOrderThreeRadius
  ...
  rw [← hgenerator]
  exact orderThreeAffineCollarLift_action Geometry.establishedPaperAnalyticData.periods
    Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
    hproper hsource Geometry.establishedPaperAnalyticData.starSeparation.orderThree.sourceData
    (cyclicGenerator 3) paperVanKampenOrderThreeCollarRepresentative
...
public theorem paperVanKampenOrderFourCollarToRegular_generator :
  ...
  let _ := restrictedMulAction
    (orderFourAffineFamilyAction Geometry.establishedPaperAnalyticData.periods)
    (orderFourAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods hsource
      paperVanKampenOrderFourRadius)
  change orderFourCollarToRegular Geometry.establishedPaperAnalyticData.periods hproper
      Geometry.establishedPaperAnalyticData.starSeparation.orderFour.sourceData
        (orderFourPuncturedCollarGaugeDiffeomorph Geometry.establishedPaperAnalyticData.periods
          Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
          paperVanKampenOrderFourRadius
          (restrictedActionMap
            (orderFourAffinePuncturedCarrier Geometry.establishedPaperAnalyticData.periods hsource
            ...
            (cyclicGenerator 4) paperVanKampenOrderFourCollarRepresentative)) =
    regularFamilyDeckMap Geometry.establishedPaperAnalyticData.periods g₂
      (orderFourCollarToRegular Geometry.establishedPaperAnalyticData.periods hproper
        Geometry.establishedPaperAnalyticData.starSeparation.orderFour.sourceData
          (orderFourPuncturedCollarGaugeDiffeomorph Geometry.establishedPaperAnalyticData.periods
            Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
            paperVanKampenOrderFourRadius
  ...
  have hgenerator : Monoid.Coprod.inr (cyclicGenerator 4) = g₂ := by
    ...
    exact SphereSixComplex.TriangleGroup.g₂.eq_def.symm
  rw [← hgenerator]
  exact orderFourAffineCollarLift_action Geometry.establishedPaperAnalyticData.periods
    Geometry.establishedPaperAnalyticData.totalSpace_projection_isLocalDiffeomorph_analytic
    hproper hsource Geometry.establishedPaperAnalyticData.starSeparation.orderFour.sourceData
    (cyclicGenerator 4) paperVanKampenOrderFourCollarRepresentative
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarRegularDeckLoop :
    Path (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
        paperVanKampenOrderThreeCentralRepresentative)
      (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
        paperVanKampenOrderThreeCentralRepresentative) :=
  (paperVanKampenOrderThreeCollarRegularDeckPath.map
      (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods).continuous).cast rfl
    (regularFamilyQuotientMap_deck Geometry.establishedPaperAnalyticData.periods
...
public theorem paperVanKampenOrderThreeCentralRepresentative_projects :
    regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
        paperVanKampenOrderThreeCentralRepresentative =
      Geometry.establishedPaperAnalyticData.starToCentral 1 paperVanKampenOrderThreeCollarPoint := by
  let _ := Geometry.establishedPaperAnalyticData.totalSpaceCharts
  let _ : IsManifold GlobalDeckTotalModel ω
      (TotalSpace (parameterMap Geometry.establishedPaperAnalyticData.periods)) :=
    Geometry.establishedPaperAnalyticData.totalSpace_isManifold_analytic
  ...
  calc
    regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
        paperVanKampenOrderThreeCentralRepresentative =
        Geometry.establishedPaperAnalyticData.orderThreePuncturedCollarToCentralFamily
          Geometry.establishedPaperAnalyticData.starSeparation.orderThree.sourceData
          (Quotient.mk _ paperVanKampenOrderThreeCollarRepresentative) := by
      exact hmk.symm
    _ = Geometry.establishedPaperAnalyticData.starToCentral 1
        (Quotient.mk _ paperVanKampenOrderThreeCollarRepresentative) := rfl
    _ = Geometry.establishedPaperAnalyticData.starToCentral 1 paperVanKampenOrderThreeCollarPoint :=
      congrArg (Geometry.establishedPaperAnalyticData.starToCentral 1) hsection
...
public theorem paperVanKampenOrderFourCentralRepresentative_projects :
    regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
        paperVanKampenOrderFourCentralRepresentative =
      Geometry.establishedPaperAnalyticData.starToCentral 2 paperVanKampenOrderFourCollarPoint := by
  let _ := Geometry.establishedPaperAnalyticData.totalSpaceCharts
  let _ : IsManifold GlobalDeckTotalModel ω
      (TotalSpace (parameterMap Geometry.establishedPaperAnalyticData.periods)) :=
    Geometry.establishedPaperAnalyticData.totalSpace_isManifold_analytic
  ...
  calc
    regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
        paperVanKampenOrderFourCentralRepresentative =
        Geometry.establishedPaperAnalyticData.orderFourPuncturedCollarToCentralFamily
          Geometry.establishedPaperAnalyticData.starSeparation.orderFour.sourceData
          (Quotient.mk _ paperVanKampenOrderFourCollarRepresentative) := by
      exact hmk.symm
    _ = Geometry.establishedPaperAnalyticData.starToCentral 2
        (Quotient.mk _ paperVanKampenOrderFourCollarRepresentative) := rfl
    _ = Geometry.establishedPaperAnalyticData.starToCentral 2 paperVanKampenOrderFourCollarPoint :=
      congrArg (Geometry.establishedPaperAnalyticData.starToCentral 2) hsection
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarToCentral :
    C(Geometry.establishedPaperAnalyticData.starCollarSourceType 1,
      Geometry.establishedPaperAnalyticData.CentralFamily) :=
  ⟨Geometry.establishedPaperAnalyticData.starToCentral 1,
    (Geometry.establishedPaperAnalyticData.starToCentral_isOpenEmbedding 1).continuous⟩

@[expose] public noncomputable def paperVanKampenOrderFourCollarToCentral :
    C(Geometry.establishedPaperAnalyticData.starCollarSourceType 2,
      Geometry.establishedPaperAnalyticData.CentralFamily) :=
  ⟨Geometry.establishedPaperAnalyticData.starToCentral 2,
    (Geometry.establishedPaperAnalyticData.starToCentral_isOpenEmbedding 2).continuous⟩
...
@[expose] public noncomputable def paperVanKampenOrderThreeFillingPoint :
    Geometry.establishedPaperAnalyticData.starFillingType 1 :=
  Geometry.establishedPaperAnalyticData.starToFilling 1 paperVanKampenOrderThreeCollarPoint

@[expose] public noncomputable def paperVanKampenOrderFourFillingPoint :
    Geometry.establishedPaperAnalyticData.starFillingType 2 :=
  Geometry.establishedPaperAnalyticData.starToFilling 2 paperVanKampenOrderFourCollarPoint
...
@[expose] public noncomputable def paperVanKampenOrderThreeFillingRepresentative :
    Geometry.establishedPaperAnalyticData.orderThreeFillingOpen paperVanKampenOrderThreeRadius := by
  let _ := Geometry.establishedPaperAnalyticData.orderThreeFillingAction
    paperVanKampenOrderThreeRadius
  exact quotientSection
    (M := Geometry.establishedPaperAnalyticData.orderThreeFillingOpen
      paperVanKampenOrderThreeRadius)
...
@[expose] public noncomputable def paperVanKampenOrderFourFillingRepresentative :
    Geometry.establishedPaperAnalyticData.orderFourFillingOpen paperVanKampenOrderFourRadius := by
  let _ := Geometry.establishedPaperAnalyticData.orderFourFillingAction
    paperVanKampenOrderFourRadius
  exact quotientSection
    (M := Geometry.establishedPaperAnalyticData.orderFourFillingOpen
      paperVanKampenOrderFourRadius)
...
@[expose] public noncomputable def paperVanKampenOrderThreeFillingDeckPath :
  ...
  let _ := Geometry.establishedPaperAnalyticData.orderThreeFillingAction
    paperVanKampenOrderThreeRadius
  let _ : PathConnectedSpace
      (Geometry.establishedPaperAnalyticData.orderThreeFillingOpen paperVanKampenOrderThreeRadius) :=
    Geometry.establishedPaperAnalyticData.orderThreeFillingOpen_pathConnected
      Geometry.establishedPaperAnalyticData.starSeparation.orderThree.radius_pos
      Geometry.establishedPaperAnalyticData.starSeparation.orderThree.radius_lt_one
...
@[expose] public noncomputable def paperVanKampenOrderFourFillingDeckPath :
  ...
  let _ := Geometry.establishedPaperAnalyticData.orderFourFillingAction
    paperVanKampenOrderFourRadius
  let _ : PathConnectedSpace
      (Geometry.establishedPaperAnalyticData.orderFourFillingOpen paperVanKampenOrderFourRadius) :=
    Geometry.establishedPaperAnalyticData.orderFourFillingOpen_pathConnected
      Geometry.establishedPaperAnalyticData.starSeparation.orderFour.radius_pos
      Geometry.establishedPaperAnalyticData.starSeparation.orderFour.radius_lt_one
...
public theorem paperVanKampenInnerRepresentative_projects :
    regularFamilyCoverProjection Geometry.establishedPaperAnalyticData.periods
...
public noncomputable def paperVanKampenRegularToCentralFundamentalGroup :
    FundamentalGroup (RegularTotalSpace Geometry.establishedPaperAnalyticData.periods)
        paperVanKampenOuterRepresentative →*
      FundamentalGroup Geometry.establishedPaperAnalyticData.CentralFamily
        (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods
          paperVanKampenOuterRepresentative) :=
  FundamentalGroup.mapOfEq
    (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods) rfl
...
public def paperVanKampenRegularPeriodGeneratorSet :
    Set (FundamentalGroup (RegularTotalSpace Geometry.establishedPaperAnalyticData.periods)
      paperVanKampenOuterRepresentative) :=
  Set.range (fun a : IntegerPeriods ↦ pathLoopClass
    ((regularFamilyPeriodLoop Geometry.establishedPaperAnalyticData.periods
...
public theorem paperVanKampenCentralFamily_twoStageCoverGenerators_generate :
  ...
  · exact regularFamilyPeriodLoops_and_vectorCoverRange_generate_of_eq
      Geometry.establishedPaperAnalyticData.periods hproper paperVanKampenInnerRepresentative
...
public theorem paperVanKampenCentralFamily_twoStageCoverGenerators_generate_of_eq
    (p : RegularBase
        ...
        ComplexTwoSpace)
    (x : RegularTotalSpace Geometry.establishedPaperAnalyticData.periods)
    (hpx : regularFamilyCoverProjection Geometry.establishedPaperAnalyticData.periods p = x) :
    let z := regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods x
    let f := FundamentalGroup.mapOfEq
      (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods) (x := x) (y := z) rfl
    let g := FundamentalGroup.mapOfEq
      (regularFamilyCoverProjection Geometry.establishedPaperAnalyticData.periods) (x := p) (y := x) hpx
    Subgroup.closure
      (({regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₁,
        regularDeckMeridian Geometry.establishedPaperAnalyticData.periods x g₂} : Set _) ∪
        f '' Set.range (fun a : IntegerPeriods ↦ pathLoopClass
          ((regularFamilyPeriodLoop Geometry.establishedPaperAnalyticData.periods p a).cast
            hpx.symm hpx.symm)) ∪
  ...
  · exact regularFamilyPeriodLoops_and_vectorCoverRange_generate_of_eq
      Geometry.establishedPaperAnalyticData.periods hproper p x hpx
...
@[expose] public noncomputable def paperVanKampenOrderThreeRegularCoverWhiskerLift :
  ...
  let _ : PathConnectedSpace
    ...
    regularBase_pathConnected
      Geometry.establishedPaperAnalyticData.modular.modularParameter.toTriangleUniformization
...
@[expose] public noncomputable def paperVanKampenOrderThreeRegularWhisker :
    Path paperVanKampenOuterRepresentative
      paperVanKampenOrderThreeCentralRepresentative :=
  (regularFamilyCoverWhisker Geometry.establishedPaperAnalyticData.periods
...
@[expose] public noncomputable def paperVanKampenOrderThreeCentralWhisker :
  ...
  exact (paperVanKampenOrderThreeRegularWhisker.map
    (regularFamilyQuotientMap Geometry.establishedPaperAnalyticData.periods).continuous).cast
...
public theorem paperVanKampenOrderThreeCollarPoint_glued :
    paperFourPieceStar.glueData.toGlueData.ι none
        (Geometry.establishedPaperAnalyticData.starToCentral 1 paperVanKampenOrderThreeCollarPoint) =
      paperFourPieceStar.glueData.toGlueData.ι (some 1)
        paperVanKampenOrderThreeFillingPoint := by
  change Geometry.establishedPaperAnalyticData.openEmbeddingStarData.toFourPieceStarGluingData.glueData.toGlueData.ι
      none
        (Geometry.establishedPaperAnalyticData.starToCentral 1 paperVanKampenOrderThreeCollarPoint) =
    Geometry.establishedPaperAnalyticData.openEmbeddingStarData.toFourPieceStarGluingData.glueData.toGlueData.ι
      (some 1)
        (Geometry.establishedPaperAnalyticData.starToFilling 1 paperVanKampenOrderThreeCollarPoint)
  exact Geometry.establishedPaperAnalyticData.openEmbeddingStarData
...
public theorem paperVanKampenOrderFourCollarPoint_glued :
    paperFourPieceStar.glueData.toGlueData.ι none
        (Geometry.establishedPaperAnalyticData.starToCentral 2 paperVanKampenOrderFourCollarPoint) =
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarCentralToCarrier :
    C(Geometry.establishedPaperAnalyticData.starCollarSourceType 1, PaperGluedCarrier) :=
  ⟨fun x => paperFourPieceStar.glueData.toGlueData.ι none
      (Geometry.establishedPaperAnalyticData.starToCentral 1 x),
...
@[expose] public noncomputable def paperVanKampenOrderThreeCollarFillingToCarrier :
  ...
  ⟨fun x => paperFourPieceStar.glueData.toGlueData.ι (some 1)
      (Geometry.establishedPaperAnalyticData.starToFilling 1 x),
    (paperFourPieceStar.glueData.ι_isOpenEmbedding (some 1)).continuous.comp
      (Geometry.establishedPaperAnalyticData.starToFilling_isOpenEmbedding 1).continuous⟩

@[expose] public noncomputable def paperVanKampenOrderFourCollarCentralToCarrier :
    C(Geometry.establishedPaperAnalyticData.starCollarSourceType 2, PaperGluedCarrier) :=
  ⟨fun x => paperFourPieceStar.glueData.toGlueData.ι none
      (Geometry.establishedPaperAnalyticData.starToCentral 2 x),
    (paperFourPieceStar.glueData.ι_isOpenEmbedding none).continuous.comp
      (Geometry.establishedPaperAnalyticData.starToCentral_isOpenEmbedding 2).continuous⟩
...
@[expose] public noncomputable def paperVanKampenOrderFourCollarFillingToCarrier :
  ...
  ⟨fun x => paperFourPieceStar.glueData.toGlueData.ι (some 2)
      (Geometry.establishedPaperAnalyticData.starToFilling 2 x),
    (paperFourPieceStar.glueData.ι_isOpenEmbedding (some 2)).continuous.comp
      (Geometry.establishedPaperAnalyticData.starToFilling_isOpenEmbedding 2).continuous⟩
    ...
    paperVanKampenOrderThreeCollarDeckLoop.map
      ...
      (paperVanKampenOrderThreeCollarDeckLoop.map
        paperVanKampenOrderThreeCollarFillingToCarrier.continuous).cast
          (Geometry.establishedPaperAnalyticData.openEmbeddingStarData
            |>.toFourPieceStarGluingData_ι_toCentral_eq_ι_toFilling
              1 paperVanKampenOrderThreeCollarPoint)
          (Geometry.establishedPaperAnalyticData.openEmbeddingStarData
            |>.toFourPieceStarGluingData_ι_toCentral_eq_ι_toFilling
...
@[expose] public noncomputable def paperVanKampenOrderThreeFillingToCarrier :
    C(Geometry.establishedPaperAnalyticData.starFillingType 1, PaperGluedCarrier) :=
  ⟨paperFourPieceStar.glueData.toGlueData.ι (some 1),
...
@[expose] public noncomputable def paperVanKampenOrderFourFillingToCarrier :
    C(Geometry.establishedPaperAnalyticData.starFillingType 2, PaperGluedCarrier) :=
  ⟨paperFourPieceStar.glueData.toGlueData.ι (some 2),
...
@[expose] public noncomputable def paperVanKampenFiberToCarrier :
  ...
  ⟨fun q => paperFourPieceStar.glueData.toGlueData.ι none
      (fiberTorusToPuncturedGlobalFamily Geometry.establishedPaperAnalyticData.periods
...
@[expose] public noncomputable def paperVanKampenTranslation :
  ...
  (FundamentalGroup.mapOfEq paperVanKampenFiberToCarrier
    ...
    (fiberTorusTranslation Geometry.establishedPaperAnalyticData.periods
      paperVanKampenRegularBasepoint paperVanKampenFiberPoint)
...
@[expose] public noncomputable def paperVanKampenTranslationLoop
    ...
    Path paperVanKampenBasepoint paperVanKampenBasepoint :=
  ((fiberPeriodLoop Geometry.establishedPaperAnalyticData.periods
...
@[instance_reducible] public noncomputable def paperFourPieceStarComplexCharts :
  ...
  Geometry.establishedPaperAnalyticData.fourPieceStarComplexCharts
...
public theorem paperFourPieceStar_pieceIsManifold :
  ...
  Geometry.establishedPaperAnalyticData.fourPieceStar_pieceIsManifold
...
@[expose] public noncomputable def paperGluingDataOfRemainingTopologicalInputs
  ...
  integralHomology :=
    hSectionSeven.toFourPieceSmallChainComparison.hasIntegralHomologyOfSixSphere
