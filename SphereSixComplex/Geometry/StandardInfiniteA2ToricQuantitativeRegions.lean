...
public structure ActualLocalCuspQuotientWitness
  ...
  matrixBound : ℝ
  matrixBound_nonneg : 0 ≤ matrixBound
  matrixEntryBound : ∀ (p : LocalCarrier M radius), M.t p ≠ 0 → ∀ i j,
    |CuspPhaseEstimates.CuspPeriodExpansion.NormalizedFuchsianCuspCoordinate.phaseLogMatrix
      N (M.t p) i j| ≤ matrixBound
  logDominates : ∀ (p : LocalCarrier M radius), M.t p ≠ 0 →
    4 * matrixBound ≤ |Real.log ‖M.t p‖|
  fixedPoint :
    (CuspPhaseEstimates.CuspPeriodExpansion.NormalizedFuchsianCuspCoordinate.restrictedActualLocalPhaseCoefficients
...
public theorem exists_actualLocalCuspQuotientWitness
  ...
  exact ⟨⟨r, hr, hrone, hradius, A, hA, hmatrix, hlog,
    hfixed, T.compactOverlapEstimate⟩⟩
...
public theorem normalizedPhaseCorrectionMatrix_entry_le
  ...
  apply (div_le_iff₀ habslog).2
  have hentry := W.matrixEntryBound p hp i j
  have hlog := W.logDominates p hp
  nlinarith
