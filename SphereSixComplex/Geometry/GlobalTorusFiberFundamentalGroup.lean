...
public noncomputable def fiberPeriodLoop
    ...
    Path (torusProjection (regularParameterMap F b).1 z)
      (torusProjection (regularParameterMap F b).1 z) := by
  let x := (regularParameterMap F b).1
  let g := fiberPeriodGroupElement F b a
  have hend : torusProjection x (g • z) = torusProjection x z := by
    apply Quotient.sound
    change MulAction.orbitRel (PeriodGroup x) ComplexTwoSpace (g • z) z
    rw [MulAction.orbitRel_apply, MulAction.mem_orbit_iff]
    exact ⟨g, rfl⟩
  exact ((fiberPeriodLiftPath F b z a).map continuous_quot_mk).cast rfl hend.symm
...
/-- Under source properness, the regular vector-bundle cover is a quotient covering of the
...
public theorem regularFamilyCoverProjection_isQuotientCoveringMap
    ...
    IsQuotientCoveringMap
      (quotientProjection
        (M := RegularBase (U := U) × ComplexTwoSpace)
        (G := FamilyPeriodGroup (regularParameterMap F)))
...
/-- Under source properness, the regular vector-bundle cover is a covering space of the regular
torus family.  This makes arbitrary geometric deck paths available to the labelled-period
transport argument, rather than only paths initially chosen upstairs. -/
...
/-- The explicit labelled period loops, together with loops coming from the regular
vector-bundle cover, generate the fundamental group of the varying torus family.  This is the
inner quotient-covering reduction used by the paper's van Kampen calculation. -/
public theorem regularFamilyPeriodLoops_and_vectorCoverRange_generate
    (hproper : SourceActionProperlyDiscontinuous (U := U))
    (p : RegularBase (U := U) × ComplexTwoSpace) :
    Subgroup.closure
      (Set.range (fun a : IntegerPeriods ↦
          pathLoopClass (regularFamilyPeriodLoop F p a)) ∪
        (FundamentalGroup.mapOfEq (regularFamilyCoverProjection F)
          (x := p) (y := regularFamilyCoverProjection F p) rfl).range) = ⊤ := by
  ...
  let S : Set (FundamentalGroup (RegularTotalSpace F)
      (regularFamilyCoverProjection F p)) :=
    Set.range (fun a : IntegerPeriods ↦
      pathLoopClass (regularFamilyPeriodLoop F p a))
  have hmono (a : IntegerPeriods) :
      hp.fundamentalGroupToMulOpposite
          (⟨p, rfl⟩ : (quotientProjection
              (M := RegularBase (U := U) × ComplexTwoSpace)
              (G := FamilyPeriodGroup (regularParameterMap F))) ⁻¹'
            {quotientProjection
              (M := RegularBase (U := U) × ComplexTwoSpace)
              (G := FamilyPeriodGroup (regularParameterMap F)) p})
          (pathLoopClass (regularFamilyPeriodLoop F p a)) =
        MulOpposite.op (Multiplicative.ofAdd a) := by
    change hp.fundamentalGroupToMulOpposite _
      (pathLoopClass (projectedOrbitDeckPath p (Multiplicative.ofAdd a)
        (regularFamilyPeriodLiftPath F p a))) = _
    exact fundamentalGroupToMulOpposite_projectedOrbitDeckPath hp p
      (Multiplicative.ofAdd a) (regularFamilyPeriodLiftPath F p a)
  have himage : Subgroup.closure
      (hp.fundamentalGroupToMulOpposite
          (⟨p, rfl⟩ : (quotientProjection
            ...
            {quotientProjection
              (M := RegularBase (U := U) × ComplexTwoSpace)
              (G := FamilyPeriodGroup (regularParameterMap F)) p}) '' S) = ⊤ := by
    apply top_unique
    intro g _
    obtain ⟨g, rfl⟩ := MulOpposite.op_surjective g
    apply Subgroup.subset_closure
    refine ⟨pathLoopClass (regularFamilyPeriodLoop F p g.toAdd),
      ⟨g.toAdd, rfl⟩, ?_⟩
    exact hmono g.toAdd
  have hgenerate := quotientCovering_generators_and_cover_range_generate hp p S himage
  convert hgenerate using 1; rfl

/-- A based version of `regularFamilyPeriodLoops_and_vectorCoverRange_generate` for a named point
of the quotient. -/
public theorem regularFamilyPeriodLoops_and_vectorCoverRange_generate_of_eq
    (hproper : SourceActionProperlyDiscontinuous (U := U))
    (p : RegularBase (U := U) × ComplexTwoSpace) (x : RegularTotalSpace F)
    (hpx : regularFamilyCoverProjection F p = x) :
    Subgroup.closure
      (Set.range (fun a : IntegerPeriods ↦ pathLoopClass
          ((regularFamilyPeriodLoop F p a).cast hpx.symm hpx.symm)) ∪
        (FundamentalGroup.mapOfEq (regularFamilyCoverProjection F)
          (x := p) (y := x) hpx).range) = ⊤ := by
  let _ : PathConnectedSpace (RegularBase (U := U)) := regularBase_pathConnected U
  let _ : MulAction (FamilyPeriodGroup (regularParameterMap F))
      (RegularBase (U := U) × ComplexTwoSpace) :=
    familyMulAction (regularParameterMap F)
  let hp := regularFamilyCoverProjection_isQuotientCoveringMap F hproper
  let S : Set (FundamentalGroup (RegularTotalSpace F) x) :=
    Set.range (fun a : IntegerPeriods ↦ pathLoopClass
      ((regularFamilyPeriodLoop F p a).cast hpx.symm hpx.symm))
  have hmono (a : IntegerPeriods) :
      hp.fundamentalGroupToMulOpposite
          (⟨p, hpx⟩ : (quotientProjection
              (M := RegularBase (U := U) × ComplexTwoSpace)
              (G := FamilyPeriodGroup (regularParameterMap F))) ⁻¹' {x})
          (pathLoopClass
            ((regularFamilyPeriodLoop F p a).cast hpx.symm hpx.symm)) =
        MulOpposite.op (Multiplicative.ofAdd a) := by
    change hp.fundamentalGroupToMulOpposite _
      (pathLoopClass
        ((projectedOrbitDeckPath
          (M := RegularBase (U := U) × ComplexTwoSpace)
          (G := FamilyPeriodGroup (regularParameterMap F))
          p (Multiplicative.ofAdd a)
          (regularFamilyPeriodLiftPath F p a)).cast hpx.symm hpx.symm)) = _
    exact fundamentalGroupToMulOpposite_projectedOrbitDeckPath_of_eq
      (M := RegularBase (U := U) × ComplexTwoSpace)
      (G := FamilyPeriodGroup (regularParameterMap F)) hp p x hpx
      (Multiplicative.ofAdd a) (regularFamilyPeriodLiftPath F p a)
  have himage : Subgroup.closure
      (hp.fundamentalGroupToMulOpposite
          (⟨p, hpx⟩ : (quotientProjection
              (M := RegularBase (U := U) × ComplexTwoSpace)
              (G := FamilyPeriodGroup (regularParameterMap F))) ⁻¹' {x}) '' S) = ⊤ := by
    apply top_unique
    intro g _
    obtain ⟨g, rfl⟩ := MulOpposite.op_surjective g
    apply Subgroup.subset_closure
    refine ⟨pathLoopClass
        ((regularFamilyPeriodLoop F p g.toAdd).cast hpx.symm hpx.symm),
      ⟨g.toAdd, rfl⟩, ?_⟩
    exact hmono g.toAdd
  have hgenerate := quotientCovering_generators_and_cover_range_generate_of_eq
    hp p x hpx S himage
  convert hgenerate using 1; rfl
