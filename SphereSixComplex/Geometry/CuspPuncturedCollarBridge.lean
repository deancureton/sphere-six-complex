...
public noncomputable def restrictActualLocalCuspQuotientWitness
  ...
  matrixBound := W.matrixBound
  matrixBound_nonneg := W.matrixBound_nonneg
  matrixEntryBound := by
    intro p hp i j
    exact W.matrixEntryBound (localCarrierInclusion M hrW p) hp i j
  logDominates := by
    intro p hp
    exact W.logDominates (localCarrierInclusion M hrW p) hp
  fixedPoint := by
    constructor
