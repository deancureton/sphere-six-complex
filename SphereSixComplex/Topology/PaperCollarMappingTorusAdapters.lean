...
/-- The exact still-missing angular fundamental domain for the actual cusp collar.  The endpoint
map is the proved `g₀` transport followed by a chosen closing identification back to the initial
fibre. -/
public structure CuspCollarIntervalClutchingData where
  fiberBase : PeriodDomain
  closeEndpoint :
    AdditiveTorus (rhoParameters g₀ fiberBase).1 ≃ₜ AdditiveTorus fiberBase.1
  quotient : IntervalClutchingQuotientData
    (A.starCollarSourceType 0) (AdditiveTorus fiberBase.1)
    ((rhoGZeroTorusHomeomorph fiberBase).trans closeEndpoint)

/-- The exact still-missing interval trivialization and angular fundamental domain for the actual
order-three affine collar. -/
public structure OrderThreeCollarIntervalClutchingData where
  quotient : IntervalClutchingQuotientData
    (A.starCollarSourceType 1) A.orderThreeTorus
    (orderThreeAffineClutchingHomeomorph A.periods)

/-- The exact still-missing interval trivialization and angular fundamental domain for the actual
order-four affine collar. -/
public structure OrderFourCollarIntervalClutchingData where
  quotient : IntervalClutchingQuotientData
    (A.starCollarSourceType 2) A.orderFourTorus
    (orderFourAffineClutchingHomeomorph A.periods)

public noncomputable def CuspCollarIntervalClutchingData.toMappingTorusHomeomorph
    (D : A.CuspCollarIntervalClutchingData) :
    A.starCollarSourceType 0 ≃ₜ
      CircleMappingTorus
        ((rhoGZeroTorusHomeomorph D.fiberBase).trans D.closeEndpoint) :=
  D.quotient.totalHomeomorphCircleMappingTorus

public noncomputable def OrderThreeCollarIntervalClutchingData.toMappingTorusHomeomorph
    (D : A.OrderThreeCollarIntervalClutchingData) :
    A.starCollarSourceType 1 ≃ₜ
      CircleMappingTorus (orderThreeAffineClutchingHomeomorph A.periods) :=
  D.quotient.totalHomeomorphCircleMappingTorus

public noncomputable def OrderFourCollarIntervalClutchingData.toMappingTorusHomeomorph
    (D : A.OrderFourCollarIntervalClutchingData) :
    A.starCollarSourceType 2 ≃ₜ
      CircleMappingTorus (orderFourAffineClutchingHomeomorph A.periods) :=
  D.quotient.totalHomeomorphCircleMappingTorus

/-- The Wang presentation transported to the actual cusp-collar homology group. -/
public noncomputable def CuspCollarIntervalClutchingData.wangPresentation
    (D : A.CuspCollarIntervalClutchingData) (k : ℕ) :
    WangHomologyPresentation
      (IntegralSingularHomology (k + 1) (AdditiveTorus D.fiberBase.1))
      (IntegralSingularHomology (k + 1) (AdditiveTorus D.fiberBase.1))
      (IntegralSingularHomology (k + 1) (A.starCollarSourceType 0))
      (IntegralSingularHomology k (AdditiveTorus D.fiberBase.1))
      (IntegralSingularHomology k (AdditiveTorus D.fiberBase.1)) :=
  circleWangPresentationOfTotalHomeomorph
    ((rhoGZeroTorusHomeomorph D.fiberBase).trans D.closeEndpoint)
    D.toMappingTorusHomeomorph k

/-- The Wang presentation transported to the actual order-three collar homology group. -/
public noncomputable def OrderThreeCollarIntervalClutchingData.wangPresentation
    (D : A.OrderThreeCollarIntervalClutchingData) (k : ℕ) :
    WangHomologyPresentation
      (IntegralSingularHomology (k + 1) A.orderThreeTorus)
      (IntegralSingularHomology (k + 1) A.orderThreeTorus)
      (IntegralSingularHomology (k + 1) (A.starCollarSourceType 1))
      (IntegralSingularHomology k A.orderThreeTorus)
      (IntegralSingularHomology k A.orderThreeTorus) :=
  circleWangPresentationOfTotalHomeomorph
    (orderThreeAffineClutchingHomeomorph A.periods)
    D.toMappingTorusHomeomorph k

/-- The Wang presentation transported to the actual order-four collar homology group. -/
public noncomputable def OrderFourCollarIntervalClutchingData.wangPresentation
    (D : A.OrderFourCollarIntervalClutchingData) (k : ℕ) :
    WangHomologyPresentation
      (IntegralSingularHomology (k + 1) A.orderFourTorus)
      (IntegralSingularHomology (k + 1) A.orderFourTorus)
      (IntegralSingularHomology (k + 1) (A.starCollarSourceType 2))
      (IntegralSingularHomology k A.orderFourTorus)
      (IntegralSingularHomology k A.orderFourTorus) :=
  circleWangPresentationOfTotalHomeomorph
    (orderFourAffineClutchingHomeomorph A.periods)
    D.toMappingTorusHomeomorph k
