...
is the general subdivision theorem.  The existing canonical barycentric prism supplies the
chain-homotopy part of that theorem; the remaining uniform smallification argument is not
currently available in Mathlib, so it is recorded below as a general theorem about arbitrary open
covers, independent of the paper's construction.
-/
...
/-- Classical small-chain approximation in its exact chain-level form.  For an open cover it
provides a smallification map and the two homotopies exhibiting the small-chain inclusion as a
chain-homotopy equivalence.  This is a general singular-chain theorem, not an input specific to
the construction of the threefold. -/
public axiom establishedOpenCoverSmallChainRetractionData
