...
This module packages the source-independent covering-space input for an affine torus family over
a twice-punctured base.  The base deck group is explicitly `FreeGroup (Fin 2)`.  Its generators
are infinite-order meridians, even when their images in an orbifold deck group have finite order.
-/
...
/-- The standard equivariant universal-cover classification for a punctured Fuchsian affine
torus family.

The two free meridians map to the order-three and order-four orbifold generators, respectively.
The conclusion supplies only the universal cover and its affine deck action; it contains no
filling relation or van Kampen conclusion. -/
public axiom establishedPuncturedGlobalFamilyEquivariantUniversalCover
