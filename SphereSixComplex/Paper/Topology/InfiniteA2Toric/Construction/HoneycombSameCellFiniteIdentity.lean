...
public theorem injective_planeTile (v : ToricLattice) (i : Fin 6) :
  ...
  all_goals
    ...
    | rw [planeTile_of_le v i p hp,
        planeTile_of_le v i q hq] at hpq
    | rw [planeTile_of_le v i p hp,
        planeTile_of_ge v i q hq] at hpq
...
public theorem honeycombLaurentFiniteIdentity_sameChart
    (v : ToricLattice) (i : Fin 6) (p q : CellSquare) :
    planeTile v i p = planeTile v i q ↔
      cellLiftCoordinates i (fun k ↦ (p.1 k : ℂ)) ∈
          monomialDomain
            (transitionMatrix (cellChart v i) (cellChart v i)) ∧
        monomial (transitionMatrix (cellChart v i) (cellChart v i))
            (cellLiftCoordinates i (fun k ↦ (p.1 k : ℂ))) =
          cellLiftCoordinates i (fun k ↦ (q.1 k : ℂ)) := by
  rw [transitionMatrix_self]
  rw [monomial_one]
  have hdomain : cellLiftCoordinates i (fun k ↦ (p.1 k : ℂ)) ∈
  ...
  constructor
  · intro hpq
    have hpq' : p = q := injective_planeTile v i hpq
    subst q
    exact ⟨hdomain, rfl⟩
  · rintro ⟨_, hpq⟩
    apply congrArg (planeTile v i)
    apply Subtype.ext
    funext k
    have h := congrArg (cellRemoveCoordinates i) hpq
    simp only [cellRemoveCoordinates_lift] at h
    exact congrArg Complex.re (congrFun h k)

public theorem cellTransitionMatrix_nextIndex (v : ToricLattice) (i : Fin 6) :
  ...
  fin_cases i <;> ext a b <;> fin_cases a <;> fin_cases b <;>
    simp [cellChart, cellNextIndex, transitionMatrix, dualMatrix,
