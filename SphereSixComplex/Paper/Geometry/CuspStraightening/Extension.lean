...
public theorem straighteningRealParameter_psiMap
  ...
  let p' : PuncturedLocalCarrier W :=
    ⟨C.psiMap lambda p, C.psiMap_preserves_t lambda p ▸ hp⟩
  let A := actualEffectiveFanDisplacementEquiv W p hp
  let A' := actualEffectiveFanDisplacementEquiv W p'.1 p'.2
  have hA : A' = A := by
    apply LinearEquiv.ext
    intro x
    rw [actualEffectiveFanDisplacementEquiv_apply, actualEffectiveFanDisplacementEquiv_apply,
  ...
  rw [← actualEffectiveFanDisplacementEquiv_symm_apply, ← actualEffectiveFanDisplacementEquiv_symm_apply]
  ...
  have hd : A.symm (effectiveFanDisplacement N (M.t p) d) = d := by
    rw [← actualEffectiveFanDisplacementEquiv_apply W p hp d]
