...
Van Kampen makes the core fundamental group surject onto that of
{uses "compact-complex-threefold"}[the glued threefold]. The cusp filling kills the last two lattice
...
The cusp relations make the fundamental group abelian. The first Hurewicz theorem therefore
identifies it with first integral homology, which vanishes. Path connectedness then gives simple
connectedness.
:::

:::theorem "cusp-fundamental-group" (parent := "fundamental-group") (lean := "SphereSixComplex.Topology.cuspOnly_mul_comm")
In any group generated by the affine core, the cusp relation and the vanishing toric translations
make all generators commute. This uses the explicit lattice monodromies and no elliptic filling
relation.
:::
...
:::theorem "local-fundamental-groups" (parent := "fundamental-group") (lean := "SphereSixComplex.QuotientCoverMapData.fundamentalGroupEquiv_natural, SphereSixComplex.toricFillingPiOneData, SphereSixComplex.cyclicAffineFillingPiOneData")
...
the normal-closure presentation of its kernel.  Collar surjectivity makes the affine core
...
The explicit one-point compactification model has one cell in degree zero and one in degree six.
Apply the classical cellular-homology comparison: every differential vanishes because adjacent
cellular groups are zero, and the degree-six group is freely generated by its unique cell.
The model is homeomorphic to the standard six-sphere.
:::
